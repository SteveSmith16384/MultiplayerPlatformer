...
public class Sheep extends AbstractLandMob {
	...
	public static void Factory(GameModule game, Block gen) { // gen == null for normal appearance
		if (game.getNumProcessInstant() < Statics.MAX_INSTANTS) {
			if (gen == null) {
				...
				// Use generator
				Sheep.Subfactory(game, gen.getWorldX(), gen.getWorldY() - (Statics.SQ_SIZE));
			}
		}
	}
	...
	private Sheep(GameModule _game, float x, float y) {
		super(_game, Statics.act.getString(R.string.sheep), x, y, Statics.SQ_SIZE, Statics.SQ_SIZE, HEALTH, MAX_FRAMES, 200, false, false, Statics.SD_PLAYERS_SIDE, true);

		a_bmp_left[0] = Statics.img_cache.getImage(R.drawable.sheep_l, Statics.SQ_SIZE, Statics.SQ_SIZE);
		a_bmp_right[0] = Statics.img_cache.getImage(R.drawable.sheep_r, Statics.SQ_SIZE, Statics.SQ_SIZE);
	...
	public void process(long interpol) {
		...
		if (dist < Statics.ACTIVATE_DIST) { // Only process if close
			...
			// Try moving
			if (this.move(x_offset * Statics.SHEEP_SPEED, 0) == false) {
				// CANT move
			...
			this.game.original_level_data.mobs.add(new SimpleMobData(AbstractMob.SHEEP, this.getWorldX(), this.getWorldY()));
		}
	...
	protected void died() {
		...
		if (this.game.new_grid.isSquareEmpty(p.x, p.y)) { // this.game.new_grid.isSquareEmpty(17, 32)
			this.game.addBlock(Block.LAMB_CHOP, p.x, p.y, false);
		}
		if (this.game.new_grid.isSquareEmpty(p.x, p.y-1)) { // this.game.new_grid.isSquareEmpty(17, 32)
			this.game.addBlock(Block.WOOL, p.x, p.y-1, false);
		}
	}
