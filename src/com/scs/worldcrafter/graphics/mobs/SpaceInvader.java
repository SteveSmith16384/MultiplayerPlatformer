...
public class SpaceInvader extends AbstractMob {
	...
	public static void Factory(GameModule game) {
		if (Statics.monsters) {
			if (game.getNumProcessInstant() < Statics.MAX_INSTANTS) {
				float x = game.root_cam.left + (Functions.rnd(0, 1) * Statics.SCREEN_WIDTH);
				float y = game.root_cam.top;// - (Statics.SCREEN_HEIGHT/2);
				if (game.getNumProcessInstant() < Statics.MAX_INSTANTS) {
					...
					if (game.new_grid.getColliders(dummy_rect).size() == 0) {
						//dummy_rect.updateGeometricState(true);
						//if (dummy_rect.getColliders(game.root_node).size() <= 0) {
						new SpaceInvader(game, x, y);
					}
	...
	private SpaceInvader(GameModule _game, float x, float y) {
		super(_game, Statics.act.getString(R.string.space_invader), x, y, Statics.INVADER_WIDTH, Statics.INVADER_HEIGHT, (byte)1, false, true, Statics.SD_ENEMY_SIDE);

		bmp[0] = Statics.img_cache.getImage(R.drawable.invader_magenta1, Statics.INVADER_WIDTH, Statics.INVADER_HEIGHT);
		bmp[1] = Statics.img_cache.getImage(R.drawable.invader_magenta2, Statics.INVADER_WIDTH, Statics.INVADER_HEIGHT);
