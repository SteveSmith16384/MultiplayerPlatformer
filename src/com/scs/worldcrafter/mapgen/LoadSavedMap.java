...
public class LoadSavedMap extends AbstractLevelData {
	...
	public ArrayList<Point> beds = new ArrayList<Point>();
	private int r_level;
	...
	public void run() {
		try {
			String text = null;
			if (Statics.GAME_MODE == Statics.GM_WORLDCRAFTER) {
				String act_filename = Statics.GetExtStorage() + "/" + filename;
				if (new File(act_filename).exists() == false) {
					throw new RuntimeException("File " + act_filename + " doesn't exist!");
				}
				text = IOFunctions.LoadText(act_filename);
			} else {//if (Statics.GAME_MODE == Statics.GM_NINJA) {
				text = IOFunctions.ReadRawText(Statics.act, r_level);
			}
			...
			if (beds.size() > 0) {
				int i = Functions.rnd(0, beds.size()-1);
				this.start_pos = beds.get(i);
				if (Statics.DEBUGGING) {
					//data[this.start_pos.x+2][this.start_pos.y] = new SimpleBlock(Block.SLIME);
					//PlatformMob pm = new PlatformMob(game);
					mobs.add(new SimpleMobData(AbstractMob.PLATFORM1, Statics.SQ_SIZE, start_pos.y * Statics.SQ_SIZE));
					//mobs.add(new SimpleMobData(AbstractMob.PLATFORM1, 200, start_pos.y * Statics.SQ_SIZE));
					
				}
			}

		} catch (Exception e) {
			AbstractActivity.HandleError(Statics.act, e);
		}
